                    ...
                    <div className='container mx-auto flex justify-between items-center h-15 transition-all duration-900 ease-in-out'>
                        ...
                        <ul className={`${openNav ? "bg-amber-50 text-amber-30 -mt-400 " : ''} md:mt-0 block z-10 gap-10 md:gap-5 space-x-4 fixed md:relative right-0 top-15 md:top-2 bg-gray-800 min-h-screen md:min-h-10 md:mr-10 md:h-10 w-40 md:w-0 text-center md:flex transition-all duration-400 ease-in-out`}>
                            ...
                            {navItems.map((item, index) => (
                                <motion.li key={item} className='hover:text-amber-400 cursor-pointer mt-10 md:mt-0' initial={{ y: -50 - index * 3 }} animate={{ y: 0 }} transition={{ y: { delay: 0.3, duration: 0.6 + index * 0.2 } }} whileHover={{ scale: 1.1, transition: { type: "spring", duration: 0.3 } }} whileTap={{ scale: 0.9 }}> {item} </motion.li>
                            ))}

                        </ul>
                        <motion.div className='flex items-center space-x-4 z-10 '>
                            <motion.div className="fas fa-home text-2xl  cursor-pointer hover:text-amber-400" onClick={handelLoginPage} whileHover={{ scale: 1.1, transition: { type: "spring", duration: 0.3 } }} whileTap={{ scale: 0.9 }} initial={{ y: -62 }} animate={{ y: 0 }} transition={{ y:{delay: .3, duration: 2} }}>
                                <IoMdContact   />
                            </motion.div>
                            <IoMenu className='md:hidden hover:text-amber-400 cursor-pointer' onClick={handelNavItems} />
                        </motion.div>
                    </div>
                </nav>
            </div>
