...
function Navbar() {
    ...
    return (
        <div className=' fixed z-10 w-full'>
            <div className='   '>
                <nav className={`${navColor ? "bg-gray-800 " : ""} ${openNav ? "bg-gray-800 " : ""}  text-white h-15  transition-all duration-600 ease-in-out `}>
                    <div className='container mx-auto flex justify-between items-center h-15 transition-all duration-900 ease-in-out'>
                        ...
                        <ul className={`${openNav ? " bg-amber-50 text-amber-30" : '-mt-220'} block z-10 gap-10 md:gap-5 space-x-4 fixed md:relative right-0 top-15 md:top-2 bg-gray-800 min-h-screen md:min-h-10 md:mr-10 md:h-10 w-40 md:w-0 text-center md:flex transition-all duration-400 ease-in-out`}>
